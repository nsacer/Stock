...
public class ViewModelUtils {
     ...
     * 无论在哪里修改ViewModel里面的MutableLiveData的值，这个页面都会收到通知（页面在活跃状态下马上收到，
     * 非活跃状态下将在变为活跃状态的那一刻收到），收到通知后调用onChanged()方法（一般是刷新视图）
     *
    ...
    public static <T extends ViewModel> T getViewModel(Fragment fragment, Class<T> viewModelClass) {
        return getViewModel(fragment.requireActivity(), viewModelClass);
    }
    ...
    public static <T extends ViewModel> T getPrivateViewModel(Fragment fragment, Class<T> viewModelClass, ViewModelStoreOwner owner) {
        return getPrivateViewModel(fragment.requireActivity(), viewModelClass, owner);
    }

    //清空存储
    public static void clear() {
        if (!viewModelMap.isEmpty()) viewModelMap.clear();
    }
}
